...
const { sendMonthlyReportWhatsApp } = require("./whatsService");
...
// Temporary, tokenized PDF storage used only while Fast2SMS fetches the
// password-protected report. Files expire automatically and are never indexed.
const _temporaryReportPdfs = new Map();

function publishTemporaryReportPdf(pdf, filename) {
  const token = crypto.randomBytes(24).toString("hex");
  const expiresAt = Date.now() + 30 * 60 * 1000;
  _temporaryReportPdfs.set(token, { pdf, filename: String(filename || "AmbikaShelf_Monthly_Report.pdf"), expiresAt });

  setTimeout(() => {
    const item = _temporaryReportPdfs.get(token);
    if (item && item.expiresAt <= Date.now()) _temporaryReportPdfs.delete(token);
  }, 30 * 60 * 1000 + 5000).unref?.();

  const base = (process.env.REPORT_PUBLIC_BASE_URL || process.env.REPORT_BASE_URL || "").replace(/\/+$/, "");
  if (!base) throw new Error("REPORT_PUBLIC_BASE_URL or REPORT_BASE_URL is required for WhatsApp PDF delivery");
  return `${base}/api/monthly-report/media/${token}`;
}
...
async function buildAndSendReport(email, info, browser) {
  ...
  if (user.monthlyReport && user.monthlyReport.email !== true && user.monthlyReport.whatsapp !== true) {
    return { status: "skipped", reason: "no delivery method is enabled" };
  }
  ...
  const monthlyReturn = monthPct(totals.portfolio);
  const assetReturns = {
  ...
  const reportSettings = user.monthlyReport || {};
  const emailEnabled = reportSettings.email !== false && !!(user.email || email);
  const whatsappEnabled = reportSettings.whatsapp === true;
  ...
  // Email receives the PDF directly. WhatsApp receives a short-lived tokenized
  // URL to the same password-protected PDF so Fast2SMS can fetch it.
  if (emailEnabled) {
  ...
  if (whatsappEnabled) {
    const pdfFilename = `AmbikaShelf_Monthly_Report_${info.name}_${info.year}.pdf`;
    const pdfUrl = publishTemporaryReportPdf(pdf, pdfFilename);
    await sendMonthlyReportWhatsApp({
      phone: reportSettings.whatsappNumber || user.mobile || user.phone,
      firstName,
      month: info.name,
      monthlyReturn,
      pdfUrl,
...
async function listRecipients() {
  ...
  const rows = await User.collection
    ...
    .project({ email: 1, monthlyReport: 1 })
    .toArray();

  const set = new Set();
  rows.forEach((u) => {
    if (u && u.email && (u.monthlyReport?.email !== false || u.monthlyReport?.whatsapp === true)) {
      set.add(String(u.email).toLowerCase().trim());
    }
  });
...
function registerMonthlyReportRoutes(app) {
  ...
  app.get("/api/monthly-report/settings", async (req, res) => {
    try {
      ...
      const user = await User.collection.findOne(
        ...
        { projection: { name: 1, email: 1, monthlyReportsEnabled: 1, monthlyReport: 1, dob: 1, mobile: 1, phone: 1 } }
      );
      ...
      res.json({
        ...
        whatsapp: !!(user.monthlyReport && user.monthlyReport.whatsapp),
        whatsappNumber: (user.monthlyReport && user.monthlyReport.whatsappNumber) || user.mobile || user.phone || ""
      });
    } catch (err) {
  ...
  app.post("/api/monthly-report/settings", async (req, res) => {
    try {
      ...
      const whatsappEnabled = req.body.whatsappEnabled === true;
      ...
      const user = await User.collection.findOne(
        { email: { $regex: "^" + escapeRegExp(email) + "$", $options: "i" } },
        { projection: { name: 1, email: 1, mobile: 1, phone: 1 } }
      );
      if (!user) return res.status(404).json({ success: false, msg: "User not found" });

      if (enabled) {
        if (!emailEnabled && !whatsappEnabled) {
          return res.status(400).json({ success: false, msg: "Select at least one delivery method" });
        }
        if (whatsappEnabled) {
          const n = whatsappNumber.replace(/[\s().-]/g, "").replace(/^\+91/, "").replace(/^91(?=\d{10}$)/, "");
          if (!/^\d{10}$/.test(n)) {
            return res.status(400).json({ success: false, msg: "Enter a valid 10-digit WhatsApp number" });
          }
        }
      ...
      await User.collection.updateOne(
        { _id: user._id },
        {
          $set: {
            ...
            "monthlyReport.whatsapp": false,
            "monthlyReport.whatsappNumber": "",
  ...
  // Short-lived tokenized PDF endpoint used by Fast2SMS to fetch the report attachment.
  app.get("/api/monthly-report/media/:token", (req, res) => {
    const item = _temporaryReportPdfs.get(String(req.params.token || ""));
    if (!item || item.expiresAt <= Date.now()) {
      _temporaryReportPdfs.delete(String(req.params.token || ""));
      return res.status(404).send("Report not found or expired");
    }
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Length", item.pdf.length);
    res.setHeader("Content-Disposition", `inline; filename="${String(item.filename).replace(/[^A-Za-z0-9._-]/g, "_")}"`);
